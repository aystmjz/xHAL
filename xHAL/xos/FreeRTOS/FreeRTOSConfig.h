...
#ifndef XHOS_TICK_RATE_HZ
#define XHOS_TICK_RATE_HZ (1000)
#endif
 ...
 *
 * 通用方法：
 *      1.configUSE_PORT_OPTIMISED_TASK_SELECTION 为 0
 * 或者硬件不支持这种特殊方法。 2.可以用于所有FreeRTOS支持的硬件
 *      3.完全用C实现，效率略低于特殊方法。
 *      4.不强制要求限制最大可用优先级数目
 * 特殊方法：
...
/* 线程本地存储指针数量配置
 * 定义每个线程可以关联的本地存储指针的最大数量 */
#define configNUM_THREAD_LOCAL_STORAGE_POINTERS   2

/* 置1：使能Newlib库的可重入性支持；置0：禁用该功能
 * 当使用Newlib标准C库时，可用于支持多线程环境下的可重入性 */
#define configUSE_NEWLIB_REENTRANT                0

/* 置1：使用自定义的SysTick中断处理函数实现；置0：使用默认实现
 * 当需要自定义系统滴答定时器中断处理逻辑时设置为1 */
#define USE_CUSTOM_SYSTICK_HANDLER_IMPLEMENTATION 1

inline void *_xmalloc(uint32_t size)
{
    void *pvReturn = NULL;
    pvReturn       = xmalloc(size);
